...
exports.uploadResume = async (req, res, next) => {
  try {
    ...
    try {
      ...
      return res.status(400).json({
        success: false,
        message: "Could not read your resume. Please upload a valid PDF or DOCX file.",
      });
    }
    ...
    if (!resumeText || !resumeText.trim()) {
      return res.status(400).json({
        success: false,
        message: "Could not extract text from your resume. Please upload a valid PDF or DOCX file.",
      });
    }
    ...
    try {
      ...
      // Continue — analysis still returned to client even if save fails
    }
